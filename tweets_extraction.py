...
for tweet in sntwitter.TwitterSearchScraper(query).get_items():
    ...
    if len(tweets) == limit:
        ...
        tweets.append([tweet.id, tweet.date, tweet.user.id, tweet.content, tweet.lang, TextBlob(tweet.content).sentiment, tweet.replyCount, tweet.retweetCount, tweet.likeCount])

df = pd.DataFrame(tweets, columns = ['ID', 'Date', 'User', 'Content', 'Language', 'SentimentAnalysis', 'ReplyCount', 'RetweetCount', 'LikeCount'])
print(df)
"""
